...
// since we assume nums1.length is always <= nums2.length, then the run time should be BIG THETA(log(N)) where
// N == nums1.length
...
public class median {
    ...
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        // we always assume nums1.length <= nums2.length, and we are going to do a binary search on the shorter array,
        ...
        // finding/ending condition: maxLeftX <= minRightY && maxLeftY <= minRightX
        // while it is NOT the above case, we should do something in the while statement, so the while statement should
        // be !(maxLeftX <= minRightY) && !(maxLeftY <= minRightX), and when you apply DeMorgan's Law, you will get
        // the answer below.
        while ((nums1[positionX - 1] > nums2[positionY]) || (nums2[positionY - 1] > nums1[positionX])) {

            // if maxLeftX > minRightY: move towards left in the X array; do binary search on the left hand side
        ...
        // if nums1.length + nums2.length is odd, then return the else part, otherwise the statement in the if part.
        if ((nums1.length + nums2.length) % 2 == 0) {
            ...
            if (positionX == 0) {
                ...
                return (Math.max(nums1[positionX - 1], nums2[positionY - 1]) + nums2[positionY]) / 2.0;
            }
